...
print("English:", en_lemma_counts_sort[:20])
print("French:", fr_lemma_counts_sort[:20])
print("Spanish:", es_lemma_counts_sort[:20])
print("Italian:", it_lemma_counts_sort[:20])

#word cloud for each language still needed
```
...
```{python}
languages = {
    "English": (nlp_en, text_en, "Alice"),
    "French":  (nlp_fr, text_fr, "Alice"),
    "Spanish": (nlp_es, text_es, "Alicia"), #only spanish uses Alicia
    "Italian": (nlp_it, text_it, "Alice")
}

for lang, (nlp_model, text, name) in languages.items():
    doc = nlp_model(text)
    
# ---- Verbs where Alice/Alicia is subject or object ----
    verbs = [
        t.head.lemma_ 
        for t in doc 
        if t.text == name and t.dep_ in ["nsubj", "dobj", "obj"]
    ]
    
# ---- Adjectives modifying Alice/Alicia ----
    adjectives = [
        child.lemma_ 
        for token in doc if token.text == name
        for child in token.children
        if child.dep_ == "amod" and child.pos_ == "ADJ" and child.lemma_.isalpha()
    ]
    
    top_verbs = Counter(verbs).most_common(5)
    top_adjs = Counter(adjectives).most_common(5)
    
    verb_table = pd.DataFrame(top_verbs, columns=["Verb", "Count"])
    adj_table = pd.DataFrame(top_adjs, columns=["Adjective", "Count"])
    
    print(f"\n--- {lang} Top 5 Verbs ---\n")
    print(verb_table)
    
    print(f"\n--- {lang} Top 5 Adjectives Modifying {name} ---\n")
    print(adj_table)
```

Across all languages, the top verbs are predominantly speech and thinking actions (*say, think, decir, preguntar, demander, pensare*). This reflects the narrative and Alice's character in the story, with some minor difference reflecting translation choices. The top adjectives are also reflective of Alice's childish portrayal (*poor, confused, petit, pobre, povero, nuovo*). Although there are some minor differences between top verbs and adjectives, there are clear patterns seen across translations.
